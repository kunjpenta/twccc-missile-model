...
def test_api_patch_ok(client):
    ...
    u = User.objects.create_user(
        username="apiuser", password="pw", is_staff=True)
...
def test_api_patch_bad_sum(client):
    ...
    # ✅ create a staff user
    u = User.objects.create_user(
        username="apiuser2", password="pw", is_staff=True)
    ...
    if "detail" in resp.json():
        assert "Weights must sum" in resp.json()["detail"]
...
def test_api_patch_radius_guard(client):
    ...
    # ✅ create a staff user
    u = User.objects.create_user(
        username="apiuser3", password="pw", is_staff=True)
